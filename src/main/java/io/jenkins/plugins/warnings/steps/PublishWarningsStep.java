...
import hudson.plugins.warnings.WarningsResultSelector;
...
public class PublishWarningsStep extends Step {
    ...
    public static class Execution extends SynchronousNonBlockingStepExecution<AbstractResultAction> {
        ...
        protected AbstractResultAction run() throws Exception {
            ...
            String url = "mywarnings";

            WarningsResultSelector selector = new WarningsResultSelector(url);
            ...
            AnalysisResult result = new AnalysisResult(run, defaultEncoding, warnings, referenceProvider, buildHistory, url);

            // TODO: why is the health descriptor persisted and not the health? (Is this due to localization?)
            WarningsHealthDescriptor healthDescriptor = new WarningsHealthDescriptor(healthy, unHealthy, getMinimumPriority());
            WarningsPipelineResultAction action = new WarningsPipelineResultAction(run, healthDescriptor, result,
                    WarningsDescriptor.getResultUrlFromId(url));
